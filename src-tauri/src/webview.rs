...
//!     is a separate invocation and keeps working regardless: `maps --help` and
//!     `maps status` need no webview at all.
//!
...
/// The Evergreen Bootstrapper — a ~2 MB download that installs the runtime. Referenced
/// only by the Windows path and the test that pins it, so it is dead weight elsewhere.
